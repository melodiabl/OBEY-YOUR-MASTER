const Discord = require("discord.js");
const { EmbedBuilder, AttachmentBuilder } = require("discord.js");
const config = require(`${process.cwd()}/botconfig/config.json`);
const canvacord = require("canvacord");
var ee = require(`${process.cwd()}/botconfig/embed.json`);
const request = require("request");
const emoji = require(`${process.cwd()}/botconfig/emojis.json`);
module.exports = {
    name: "obama",
    aliases: [""],
    category: "🕹️ Fun",
    description: "COMANDO DE IMAGEN",
    usage: "obama @Usuario <TEXT>",
    type: "usertext",
    run: async (client, message, args, cmduser, text, prefix) => {
        let es = client.settings.get(message.guild.id, "embed");
        let ls = client.settings.get(message.guild.id, "language");
        if (!client.settings.get(message.guild.id, "FUN")) {
            return message.reply({
                embeds: [
                    new EmbedBuilder()
                        .setColor(es.wrongcolor)
                        .setFooter(client.getFooter(es))
                        .setTitle(client.la[ls].common.disabled.title)
                        .setDescription(
                            require(`${process.cwd()}/handlers/functions`).handlemsg(
                                client.la[ls].common.disabled.description,
                                { prefix: prefix }
                            )
                        ),
                ],
            });
        }
        //send loading message
        var tempmsg = await message.reply({
            embeds: [
                new EmbedBuilder()
                    .setColor(ee.color)
                    .setAuthor({ name: "Obteniendo datos de imagen...", iconURL: "https://images-ext-1.discordapp.net/external/ANU162U1fDdmQhim_BcbQ3lf4dLaIQl7p0HcqzD5wJA/https/cdn.discordapp.com/emojis/756773010123522058.gif" }),
            ],
        });
        //get pinged user, if not then use cmd user
        var user = message.mentions.users.first();
        //if user pinged, shift the args,
        if (user) args.shift();
        //else not and define the user to be message.author
        else user = message.author;
        //get avatar of the user
        var avatar = user.displayAvatarURL();
        //get the additional text
        var text = args.join(" ");
        //If no text added, return error
        if (!text)
            return tempmsg
                .edit({
                    embeds: [
                        tempmsg.embeds[0]
                            .setTitle(eval(client.la[ls]["cmds"]["fun"]["obama"]["variable2"]))
                            .setColor("#ED4245")
                            .setDescription(eval(client.la[ls]["cmds"]["fun"]["obama"]["variable3"])),
                    ],
                })
                .catch(() => {});

        //get the memer image
        client.memer.obama(text, avatar).then(image => {
            //make an attachment
            var attachment = new AttachmentBuilder(image, "obama.png");
            //delete old message
            tempmsg.delete();
            //send new Message
            message
                .reply({
                    embeds: [
                        EmbedBuilder.from(tempmsg.embeds[0]).setAuthor({ name: `Meme para: ${user.username}`, iconURL: avatar }).setImage("attachment://obama.png"),
                    ],
                    files: [attachment],
                })
                .catch(() => {});
        });
    },
};
/**
 * @INFO
 * Desarrollado por Melodia | https://github.com/melodiabl
 * @INFO
 * Desarrollado por Melodia | https://github.com/melodiabl
 * @INFO
 * Desarrollado por Melodia | https://github.com/melodiabl
 * @INFO
 */
