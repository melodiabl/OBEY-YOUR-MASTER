...
module.exports = {
  DESCRIPTION: 'Muestra tu inventario (persistente).',
  ALIASES: ['inv'],
  async execute (client, message) {
    const inv = await Systems.items.getUserInventory({ userID: message.author.id }).catch(() => [])
    ...
    if (!lines.length) lines.push(`${Emojis.dot} ${Format.italic('Vacio')}`)
