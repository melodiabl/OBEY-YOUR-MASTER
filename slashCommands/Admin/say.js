...
module.exports = {
    ...
    description: "Envía un texto al chat", //the command description for Slash Command Overview
    cooldown: 5,
    options: [
        ...
        { String: { name: "text", description: "Qué debería enviar? [ +n+ = Nueva línea ]", required: true } }, //to use in the code: interacton.getString("title")
    ...
    run: async (client, interaction, cmduser, es, ls, prefix, player, message) => {
        try {
            ...
            await interaction?.reply({ content: `Enviando el mensaje...`, ephemeral: true }).catch(() => {});
            //SEND THE EMBED!
            await channel.send({ content: String(Text).substring(0, 2000).split("+n+").join("\n") }).catch(() => {});
            //Edit the reply
            interaction?.editReply({ content: `✅ Mensaje enviado en ${channel}!`, ephemeral: true }).catch(() => {});
        } catch (e) {
